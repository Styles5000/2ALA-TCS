/****************************************************************
Jullien Thur, Jackson Bryant, & Daniel Bohanon
ME EN 1010 Lab Section [16]            
*******************************************************************/
...
int writeToServo[6] = {0,0,0,0,0,0};
/****************************
...
float LaunchAngle(float* d, float kappa, float lambda, float xTarget){
	float d1 = d[0];
	float d2 = d[1];
	float thetaLaunch = RangeAngle(d, kappa, lambda); //starting angle for the while loop/
	float xLand = LandingDistance(d, kappa, lambda, thetaLaunch); // holds the landing distance.
	while ( xLand > xTarget){
		//increments thetaLaunch and computes new xLand value
		thetaLaunch = thetaLaunch + 0.05;
		xLand = LandingDistance(d, kappa, lambda, thetaLaunch);
	}
	return thetaLaunch; //returns final, optimized launch angle.
}

double ThetaServo(double h[], float thetaL, float alpha, float beta, float initialThetaL){
...
void TargetServoAngles(float* d, float kappa, float lambda, double h[], float alpha, float beta, float initialThetaL, float xTargetVec[]){
	...
	for (int i = 0; i<6; i++){
		...
		localServoVec[i] = ThetaServo(h, localLaunchVec[i], alpha, beta, initialThetaL);
		int ServoAngle = int(round(localServoVec[i]));
...
void setup(void) {
	...
	// initialize veriables (hardcoded)
	float d[2] = {0.0876, 0.1190};
	float kappa = 2.90;
	float lambda = -0.001;
	float xTarget = 1.20;
	float xTargetVec[6] = {0.5,0.6,0.7,0.8,0.9,1.0};
	double h[4] = {0.0866, 0.0750, 0.1060, 0.0250};
	float thetaL = 52.85;
	float alpha = 15.12;
	float beta = 0.0001;
	float initialThetaL = 28.03;
	//initialize variables based on the above functions
	float rangeAngle = RangeAngle(d, kappa, lambda);
	float range = LandingDistance(d, kappa, lambda, rangeAngle);
	float launchAngle = LaunchAngle(d, kappa, lambda, xTarget);
	double thetaS = ThetaServo(h, thetaL, alpha, beta, initialThetaL);
	Serial.print("\nThe range is ");
	Serial.print(range);
	Serial.print(" m at an angle of ");
	Serial.print(rangeAngle);
	Serial.println(" degrees.");
	if (xTarget <= range){ // this means the target can be hit.
	...
	TargetServoAngles(d, kappa, lambda, h, alpha, beta, initialThetaL, xTargetVec);
	for (int i = 0; i<6; i++){
		Serial.print("To hit a target at ");
		Serial.print(xTargetVec[i]);
